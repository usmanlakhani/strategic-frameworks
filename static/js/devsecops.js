import allJSONData from '/json/devsecops.json' assert {type: 'json'};
 

window.onload = function (){   
    
    //**** Print Topic */
    document.getElementById("topic").innerText = "DevSecOps Strategic Technical Framework";
    
    const allArrays = splitData(allJSONData);
   
    renderBusiness(allArrays[0],allJSONData[4]);
    renderIT(allArrays[1],allJSONData[4]);
    renderCapabilities(allArrays[2],allJSONData[4]);
    renderEnablers(allArrays[3],allJSONData[4]);
    
}

function splitData(allJSONData)
{
    ...
    for (let i=0;i<allJSONData.length; i++){
        ...
        const type = allJSONData[i].type;        

        if (type === 'biz')
            bizArray.push(allJSONData[i]);

        if (type === 'its')
            itArray.push(allJSONData[i]);

       if (type === 'cap')
            capArray.push(allJSONData[i]);

        if (type === 'enabler')
            enablersArray.push(allJSONData[i]);

        if (type === 'desc')
            descriptionArray.push(allJSONData[i]);
    }
