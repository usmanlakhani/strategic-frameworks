import allData from '/json/empexp.json' assert {type: 'json'};
 

window.onload = function (){   
    
    //**** Print Topic */
    document.getElementById("topic").innerText = "Employee Experience Strategic Technical Framework";
    
    const allArrays = splitData(allData);
   
    renderBusiness(allArrays[0],allData[4]);
    renderIT(allArrays[1],allData[4]);
    renderCapabilities(allArrays[2],allData[4]);
    renderEnablers(allArrays[3],allData[4]);
...
function splitData(allData)
{
    ...
    for (let i=0;i<allData.length; i++){
        ...
        const type = allData[i].type;        

        if (type === 'biz')
            bizArray.push(allData[i]);

        if (type === 'its')
            itArray.push(allData[i]);

       if (type === 'cap')
            capArray.push(allData[i]);

        if (type === 'enabler')
            enablersArray.push(allData[i]);
